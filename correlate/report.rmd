...
# 2015-09-15: adjusted to upstream changes in alignment input format
...
read.alignments_tsv<-function(alignments_tsv) {
  ...
  alignments<-rbind(data.frame(alignments[(grep("intron_5",alignments[,21])),]
                              ,upstream.type="intron"
                              ,downstream.type="exon"
                              )
                   ,data.frame(alignments[(grep("intron_3",alignments[,21])),]
                              ,upstream.type="exon"
                              ,downstream.type="intron"
                              )
                   )
  alignments$upstream.idx<-alignments[,9]-(alignments$upstream.type!="exon")
  alignments$downtream.idx<-alignments[,9]
  return(alignments)
}
...
coverage.upstream<-setNames(unlist(alply(alignments
                                        ,1                                                    # per alignment (i.e. row-wise)
                                        ,function(alignment)
                                          coverage[(coverage[,1]==alignment[1,6])             # coverage txid == aligned target txid
                           ...
                           ,unlist(alply(cbind(alignments[,6]           # label with aligned target txid
...
coverage<-setNames(cbind(alignments[,c(1,6)]
